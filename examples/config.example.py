# Copy this file to config.py
API_NCE_USER =  "your_nce_api_user"
API_NCE_PASS = "your_nce_password"
API_NCE_HOST = "https://your_nce_host:26335"
API_NCE_NE_NAME = "your_nce_network_element"  # Network element name for NCE examples
API_NCE_SUBNET_NAME = "your_subnet_name"  # Subnet name for NCE examples

API_NSP_USER =  "your_nsp_api_user"
API_NSP_PASS = "your_nsp_password"
API_NSP_HOST = "https://your_nsp_host"
API_NSP_NE_NAME = "your_nsp_network_element"  # Network element name for NSP examples

NETCONF_HOST = "192.168.0.1"
NETCONF_PORT = 830 # 22 for huawei
NETCONF_USER = "your_netconf_user"
NETCONF_PASSWORD = "yout_netconf_password"